...
from .forms import (
    ...
    ResendActivationCodeForm, ResendActivationCodeViaEmailForm, ChangeProfileForm, ChangeEmailForm, RedeemForm, ShopForm
)
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_USER_ACTIVATION:
            ...
            messages.success(
                request, _('You are signed up. Log In!'))
        else:
...
class RedeemView(LoginRequiredMixin, FormView):
    template_name = 'accounts/profile/redeem.html'
    form_class = RedeemForm

    def get_initial(self):
        user = self.request.user
        initial = super().get_initial()
        # initial['money'] = user.profile.money
        return initial

    #@csrf_exempt
    def form_valid(self, form):
        user = self.request.user
        if user.profile.voucher == False:
            # Race Condition
            user.profile.money += 20
            user.save()
            # Faking very long Database connection
            sleep(5)
            user.profile.voucher = True
        user.save()

        messages.success(self.request, _('Voucher has been successfully Redeemed! You got 20 Bucks.'))

        return redirect(settings.LOGIN_REDIRECT_URL)


class ShopView(LoginRequiredMixin, FormView):
    template_name = 'accounts/profile/shop.html'
    form_class = ShopForm

    def form_valid(self, form):
        user = self.request.user
        if user.profile.money >= 100:
            # Race Condition
            user.profile.money -= 100
            user.save()
            messages.success(self.request, _('Secret has been successfully Bought! Secret: ' + os.getenv('SECRET')))
        else:
            message.warning(self.request, _('Secret has been successfully Bought! Ah no wait.. just fooling! you are to poor!'))
        return redirect(settings.LOGIN_REDIRECT_URL)
